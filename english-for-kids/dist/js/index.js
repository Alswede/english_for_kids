...
// Burger 	// (Need solution for menu and togle (TO DO!)
...
/* Обработчик вкладки меню. Оно должно закрываться при клике на любую точку страницу, лежащую вне меню*/
...
let menuWrapper = document.querySelector('.menuToggle')
let menu =  document.querySelector('.menu')

document.addEventListener('click', (event) => {
     if (!(event.target === menuWrapper || menuWrapper.contains(event.target)) && menuWrapper.classList.contains('menuToggleActive')) {
        menuWrapper.classList.remove('menuToggleActive');
...
// Enter for play or train mode-switcher
...
document.querySelectorAll('.header-item').forEach(key => {
    key.addEventListener('click', (e) => {
        ...
        if (!e.currentTarget.classList.contains('active') && e.currentTarget.innerHTML !== document.querySelector('.header-item').innerHTML) {
            document.querySelector('.additional-container').classList.remove('none');
            document.querySelector('.main-container').classList.add('none');
            document.querySelectorAll('.front').forEach(key => {
                k = count[i];
                key.style.backgroundImage = cards[page][k].image;
                i++;
            });
            i = 0;
            ...
            document.querySelectorAll('.card-header-front').forEach(key => {
                k = count[i];
                key.innerHTML = cards[page][k].word;
                i++;
            });
            i = 0;
            document.querySelectorAll('.card-header-back').forEach(key => {
                k = count[i];
                key.innerHTML = cards[page][k].translation;
                i++;
            });
            i = 0;
        } else {
            if (e.currentTarget.innerHTML === document.querySelector('.header-item').innerHTML) {
                document.querySelector('.additional-container').classList.add('none');
...
document.querySelectorAll('.main-card').forEach(key => {
    key.addEventListener('click', (e) => {
        ...
        document.querySelectorAll('.card-header-front').forEach(key => {
            k = count[i];
            key.innerText = cards[page][k].word;
            i++;
        });
        i = 0;
        document.querySelectorAll('.card-header-back').forEach(key => {
            k = count[i];
            key.innerText = cards[page][k].translation;
            i++;
        });
        i = 0;
        document.querySelector('.active').classList.remove('active');
...
document.querySelector('.btn').addEventListener('click', () => {
    ...
    document.querySelector('.sound').src = sound[realSound]
})
...
document.querySelectorAll('.front').forEach(key => {
    ...
    key.addEventListener('click', (e) => {
        if (e.currentTarget.classList.contains('front-play')) {
            ...
            if (e.currentTarget.getAttribute('num') === cards.sound[sound[realSound]]) {
                star = document.createElement('div');
                e.currentTarget.classList.add('inactive');
                e.currentTarget.classList.remove('front-play');
                document.querySelector('.sound').src = sound[realSound + 1];
                ...
                realSound++;
                correct++;
				// If all cards correct - enter to Main Menu. After GameOver.For give result and finish game. Happy or unhappy smile and sound.
                if (correct === 8) {
                    revertBackToMainMenu();
                }
            } else {
                star = document.createElement('div');
...
// Revert (To DO: add: Rating)
function revertBackToMainMenu() {
...
// This function for mix and shuffle
function shuffle(squareNumberArray) {
    let currentIndex = squareNumberArray.length, tempValue, tempIndex;
    while (0 !== currentIndex) {
        tempIndex = Math.floor(Math.random() * currentIndex);
        currentIndex -= 1;
        tempValue = squareNumberArray[currentIndex];
        squareNumberArray[currentIndex] = squareNumberArray[tempIndex];
        squareNumberArray[tempIndex] = tempValue;
    }
...
// Set of cards (with const: word, translation, image, audio)
const cards = {
    sound: {},
